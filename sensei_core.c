...
/*
 * Sensei Core — Central Brainstem
 *
 * Cortex implementations live in sensei_core/all_c_files
 * This file wires the public API to cortex modules and seeds capabilities.
 */
...
/* Capability + dispatch functions */
bool sensei_cap_register(const char *worker, const char *command);
bool sensei_dispatch(const sensei_msg_t *msg, const sensei_route_result_t *route);

/* One‑time capability seeding */
...
static void sensei_core_ensure_caps(void) {
    ...
    /* Same routing semantics as your old static table */
