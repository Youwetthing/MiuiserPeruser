...
# ═══════════════════════════════════════════════════════════════════════════════
# syndicate_scan.sh — MiuiserPeruser Full Syndicate Scan v6.1
# Fixes applied: extra_env bug (was silent no-op via `env`), INT trap killed
# whole script (now returns to menu), rolling_scan is parallel (not sequential),
# shredderd parser uses 1 jq call instead of 4 cold python3 spawns and reads
# the correct .drift.new_modules path (was silently reading .integrity.new_modules
# which never existed), nulld wired into the fleet.
# ═══════════════════════════════════════════════════════════════════════════════
...
declare -A DAEMON_SIMPLE=(
  [burned]="Detects factory-level tracking baked into your Xiaomi firmware. Partner IDs embedded at ROM level."
  [granitord]="Checks security foundations — SELinux, verified boot, tamper status."
  [leatherheadd]="Monitors thermal state — heat throttling to prevent damage."
  [metalheadd]="Watches sensors for background polling."
  [rahzerd]="Audits all network connections — WiFi, mobile, DNS, open ports."
  [ratkingd]="Tracks memory pressure, zombie processes, orphans."
  [rocksteadyd]="Monitors CPU frequency and throttling."
  [shredderd]="Verifies kernel integrity — unexpected modules, rootkit signatures."
  [tigerclawd]="Maps Binder service registry — HyperOS internal plumbing."
  [bebopd]="Measures battery drain rate."
  [fugitoidd]="Parses crash logs — FATAL EXCEPTION, ANR, OOM."
  [overlordd]="Cross-daemon correlation engine."
  [nulld]="Watches what runs on your phone while the screen is off — flags apps quietly sending data during idle."
)

declare -A DAEMON_TECHNICAL=(
  [burned]="Scans persist.sys.miui_optimization, ro.* advertiser flags, GDPR opt-out. Detects Facebook/AppsFlyer/Xiaomi partner IDs in build.prop. Cannot remove without flashing."
  [granitord]="SELinux mode, ro.secure, ro.debuggable, verified boot, dm-verity, bootloader lock. Score across 7 vectors."
  [leatherheadd]="MTK thermal zones via /sys/class/thermal/. Throttle: scaling_cur_freq < 60% max_freq for 3+ polls. Score = 100 - (temp + throttle + zones)."
  [metalheadd]="/proc/[pid]/fd scan for sensor nodes. Classifies: ACCEL, GYRO, MAG, PROX, LIGHT. Background flagged when no visible Activity."
  [rahzerd]="WiFi via cmd wifi status, mobile RAT via dumpsys telephony.registry, DNS via getaddrinfo(), TCP from /proc/net/tcp{,6}."
  [ratkingd]="Zombies: State Z in /proc/[pid]/status. Orphans: PPID=1. Memory: /proc/meminfo. OOM: logcat grep 'am_kill'."
  [rocksteadyd]="Per-core freq via scaling_cur_freq. Throttle: cur_freq < 0.6*max_freq for 3+ polls. Score = unthrottled/total*60 + stability*30 + gov*10."
  [shredderd]="/proc/modules inventory. Baseline DJB2 hash. Rootkit sigs: su, hide, xposed, magisk, riru, zygisk, lsposed. Score = 100-(new*20 + rootkit*30 + sysfs*10)."
  [tigerclawd]="Binder registry via service list. Baseline: 473 services. Drift: DJB2 hash of service names. Trust = service(30)+prop(25)+patch(20)+hyperos(15)+selinux(10)."
  [bebopd]="Drain: /sys/class/power_supply/battery/charge_now delta. mAh/hr = charge_delta/elapsed. Grades: A<200, B<400, C<600, D<800, F>800."
  [fugitoidd]="FATAL: logcat -b crash | grep FATAL. ANR: logcat -b events | grep 'am_anr'. OOM: logcat -b events | grep 'am_kill'. Dedup by (package,signal)."
...
# ── Per-daemon extra env, built as an array (fixes the $extra_env word-splitting bug) ──
_daemon_env() {
    case "$1" in
...
# ═══════════════════════════════════════════════════════════════════════════════
# ROLLING SCAN — parallel launch, live feed as each daemon finishes
# ═══════════════════════════════════════════════════════════════════════════════
rolling_scan() {
    local round=1
...
# ═══════════════════════════════════════════════════════════════════════════════
# PAGED RESULTS — All daemons, prev/next, JSON box + simple + technical
# ═══════════════════════════════════════════════════════════════════════════════
show_paged_results() {
    local daemon_list=()
    for d in $ENABLED; do
...
run_fast_scan() {
    ...
    printf "  ${GRN}✔${RST} ${result_count}/${total} daemons produced results\n\n"
}
...
main() {
    ...
    while true; do
        ...
        printf "\n${BOLD}${WHT}"
        printf "╔══════════════════════════════════════════════════════════════════════╗\n"
        printf "║           MIUISERPERUSER — SYNDICATE SCAN v6.1                       ║\n"
        printf "║           Rolling (parallel) · Paged · Fast                          ║\n"
        printf "╚══════════════════════════════════════════════════════════════════════╝\n"
        printf "${RST}\n"
        ...
        printf "  ${CYN}[3]${RST} ${WHT}Fast Scan (parallel)${RST}   ${DIM}(all at once, quick batch)${RST}\n"
        ...
        case "$choice" in
            ...
            3)
                run_fast_scan 1
                printf "\n  ${CYN}Press any key for paged results...${RST}"
                read -n1
                show_paged_results
                ;;
            4)
                ...
                printf "  ${CYN}Press any key for paged results...${RST}"
                read -n1
                show_paged_results
                ;;
            5)
